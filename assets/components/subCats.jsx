...
console.log(subCategories.subCategories);
  ...
  return (
    <div className='p-4'>
       <h2 className='text-lg uppercase text-center underline my-4'> {subCategories.name} </h2>
    <ul className='w-full inline-block'>
      { subCategories.subCategories && subCategories.subCategories.map(cat => { 
        return(<li className='flex items-center float-left w-1/4 cursor-pointer' key={cat.id} onClick={() => select(cat)}>
