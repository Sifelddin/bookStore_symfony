...
const getUrlPerPage = (hydraView) => data['hydra:view'][hydraView] && setPageUrl(data['hydra:view'][hydraView])
...
const addStyles = (cat) =>{
  ...
  return (
    <div className='p-3 bg-orange-50 my-5 shadow-md rounded-md'>
       <span className='uppercase text-gray-500 pl-4'>total categories : <strong>{data['hydra:totalItems']}</strong></span>
      <div className='grid xl:grid-cols-2 xl:gap-2 p-2 w-full border-b-2 '>
      {data['hydra:member'].map((cat, i) => {
        return(
          <div className='flex items-center cursor-pointer' key={cat.id}  onClick={() =>{select(cat)}}>
          <img id={cat.id} className="w-16 h-16 rounded-full m-0 border-4 border-white shadow-md " ref={addToRefs} onClick={(e) => addStyles(e)} src={"uploads/images/" + cat.photo} alt={cat.title} />
